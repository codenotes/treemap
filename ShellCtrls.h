...
inline BOOL AtlGetFilePidl(LPCTSTR pstrFileName, LPITEMIDLIST* pidl)
{
   ...
   *pidl = NULL;
   ...
   if( FAILED( spFolder->ParseDisplayName(NULL, NULL, T2OLE(pstrFileName), NULL, pidl, NULL) ) ) return FALSE;   
   return TRUE;
}

inline BOOL AtlBindToIDListParent(LPCITEMIDLIST pidl, IShellFolder** ppFolder, LPITEMIDLIST* pidlChild)
{
   ATLASSERT(pidl);
   ATLASSERT(ppFolder);
   ATLASSERT(pidlChild);
   ...
   *pidlChild = NULL;
   ...
   while( --nCount > 0 ) {
      ...
      CPidl pidlTemp;
      pidlTemp.Copy( pidlMain.GetNextItem() );
      pidlMain.Attach( pidlTemp.Detach() );
   }

   *ppFolder = spFolder.Detach();
   *pidlChild = pidlMain.Detach();
   return TRUE;
}
...
public:
   ...
   BOOL Populate(LPCTSTR pstrPath)
   {
      ...
      CPidl pidl;
      ULONG uEaten = 0;
      if( FAILED( spDesktop->ParseDisplayName(NULL, NULL, T2OLE(pstrPath), &uEaten, &pidl, NULL) ) ) return FALSE;
      return Populate(pidl);
   }
   ...
   BOOL Populate(LPCITEMIDLIST pidl)
   {
      CPidl pidlItem;
      CComPtr<IShellFolder> spFolder;
      if( !AtlBindToIDListParent(pidl, &spFolder, &pidlItem) ) return FALSE;
      return Populate(pidl, spFolder, pidlItem);
   }

   BOOL Populate(LPCITEMIDLIST pidlPath, IShellFolder* pParent, LPCITEMIDLIST pidlNode)
   {
      ATLASSERT(pParent);
      ATLASSERT(pidlPath);
      CComPtr<IShellFolder> spFolder;
      DWORD dwAttribs = SFGAO_FILESYSANCESTOR | SFGAO_HASSUBFOLDER;
      if( pParent != NULL && !CPidl::PidlIsEmpty(pidlNode) ) {
         // Get the new IShellFolder object
         if( FAILED( pParent->BindToObject(pidlNode, NULL, IID_IShellFolder, (LPVOID*) &spFolder) ) ) return FALSE;
         // Get this folder's attributes
         pParent->GetAttributesOf(1, &pidlNode, &dwAttribs);
      }
      else {
         // Folder is Desktop
         if( FAILED( ::SHGetDesktopFolder(&spFolder) ) ) return FALSE;
   ...
   BOOL GetItemPath(TItem hItem, LPTSTR pstrPath)
   {
      ...
      // BUG: SHGetPathFromIDList() only work on file-system paths
...
public:
   ...
   BOOL SelectPidl(LPCITEMIDLIST pidlTarget)
   {
      ...
      //      or at least the root visible from pidl.
      CPidl pidlItem;
      HTREEITEM hItem = GetRootItem();
      ATLASSERT(hItem!=NULL);
      if( hItem != NULL ) {
         DWORD_PTR lParam = GetItemData(hItem);
         ATLASSERT(lParam!=0 && lParam!=(DWORD)-1);
         PSHELLITEMINFO pItem = reinterpret_cast<PSHELLITEMINFO>(lParam);
         int nRootItems = (int) CPidl::PidlGetCount(pItem->pidlFull);
         while( --nRootItems >= 0 ) pidlTarget = CPidl::PidlGetNextItem(pidlTarget);
      }
   ...
   BOOL _Populate(IShellFolder* pFolder, LPCITEMIDLIST pidlPath, DWORD dwAttribs)
   {
      ...
      ATLASSERT(pidlPath);
      DeleteAllItems();
      HTREEITEM hItem = _InsertItem(pFolder, pidlPath, NULL, dwAttribs, TVI_ROOT);
      SelectItem(hItem);
      Expand(hItem);
      return hItem != NULL;
   }

   //inserts all items in exporer list
   ...
   {
	   //#debug called a bazillion times
	   //ODT(__FUNCTION__);
      ...
      if( SUCCEEDED(Hr) ) return (SHORT) (Hr & 0xFFFF);
      return 0;
   }
   ...
   {
	   //#debug:this is a called a bazillion times!
   ...
   LRESULT OnItemExpanding(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled)
   {
      ...
      if( SUCCEEDED( spFolder->EnumObjects(NULL, dwEnumFlags, &spEnum) ) && spEnum != NULL ) {
         CPidl pidl;
         ULONG uFetched = 0;
         while( (spEnum->Next(1, &pidl, &uFetched) == S_OK) && (uFetched > 0) ) {
...
public:
   ...
   BOOL _Populate(IShellFolder* pFolder, LPCITEMIDLIST pidlPath, DWORD dwAttribs)
   {
      ...
      if( (m_dwShellStyle & SCT_EX_NOROOT) == 0 ) {
         _InsertItem(pFolder, pidlPath, NULL, dwAttribs, iIndent++);
      }
      ...
      if( SUCCEEDED( pFolder->EnumObjects(NULL, dwEnumFlags, &spEnum) ) && spEnum != NULL ) {
         ...
         while( (spEnum->Next(1, &pidl, &uFetched) == S_OK) && (uFetched > 0) ) {
            ...
            DWORD dwAttribs = 0;
            if( !_FilterItem(pFolder, pidl, dwAttribs) ) {
               _InsertItem(pFolder, pidlPath, pidl, dwAttribs, iIndent);
            }
            pidl.Delete();
         }
      }
...
public:
   ...
   LRESULT OnShellMenuMsg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
   {
      ...
      if( m_spCtxMenu2 != NULL ) m_spCtxMenu2->HandleMenuMsg(uMsg, wParam, lParam);
      return 0;
   }
};
