      ...
      while( bRes && !IsAborted() ) 
      {
         if( (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) != 0 ) {
            ...
            if( wfd.cFileName[0] != '.' ) {
               ...
               aPaths.Add(NewPath);
			   ODT(__FUNCTION__, "PATH", NewPath.sPath); //fname and num bytes, confirmed that is what that is
            ...
            // Process file item...
			 ODT(__FUNCTION__, wfd.cFileName, wfd.nFileSizeLow); //fname and num bytes, confirmed that is what that is
...
void mindump(CTreeMapModel* pTreeModel);

void _mindump(CTreeMapItem* pt)
{
	for (int i = 0; i < pt->m_aChildren.GetSize(); i++)
	{
		_mindump(pt->m_aChildren[i]);
	}
	ODT("--->", __FUNCTION__, pt->m_Info.szName, pt->m_aFiles.m_nSize);
}

void mindump(CTreeMapModel* pTreeModel)
{
	_mindump(pTreeModel->m_pRoot);
}
...
LRESULT CTreeMapView::OnTreeUpdate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
{
   ...
   CRect rcClient;
   GetClientRect(&rcClient);
   ODT(__FUNCTION__, rcClient.bottom);
