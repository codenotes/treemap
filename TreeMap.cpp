...
CTreeMapModel::CTreeMapModel() : 
   ...
   m_hFont(NULL)
{
	//this is created for seemingly every square or file?
	//ODT("I live");
}
...
//runs the calcsize and sets up the recursive tree in CTreeMapItem* m_pRoot;  
void CTreeMapModel::Prepare()
{
   ...
   _DoClearCalculation(m_pRoot);//just zeros everything

   MINMAXPAIR pair = { 999999999999.0, 0.0 };
   m_MinMaxSize = pair;
   _DoSizeCalculation(m_pRoot);//kickoff point here I think...
...
void CTreeMapModel::DoLayout(RECT rc)
{
   ...
   //#debug, so yeah...this guy has all the children and sizes (i think) by this point.
 //  ODT("EXITING",__FUNCTION__, m_pRoot->m_Info.szName, m_pRoot->m_aChildren[0]->m_Info.szName);
   ODT(__FUNCTION__, rc.bottom,rc.left, rc.right, rc.top) //1030 ,  0 ,  2282 ,  0 
}
...
{//#debug called a lot, not sure why or when yet
	//ODT(__FUNCTION__);
...
//recursive size calculator, action seems to be here
double CTreeMapModel::_DoSizeCalculation(CTreeMapItem* pItem)
{
   // Determine the sum of size of files and folders for each node.
   double dblSize = 0.0;
   for (int i1 = 0; i1 < pItem->m_aFiles.GetSize(); i1++)
   {
	   dblSize += pItem->m_aFiles[i1].dblSize;
	   ODT("{", pItem->m_aFiles[i1].dblSize, "}");
   }

		//ODT("name:", pItem->m_Info.szName);

   for (int i2 = 0; i2 < pItem->m_aChildren.GetSize(); i2++)
   {
	   dblSize += _DoSizeCalculation(pItem->m_aChildren[i2]);
	//	ODT(__FUNCTION__,  dblSize);
   }
...
void CTreeMapModel::_DoLayout(CTreeMapItem* pItem, RECT rc)
{
   ...
   ODT(pItem->m_Info.szName, pItem->m_Info.dblSize, pItem->m_Info.rc.bottom, rc.right,rc.left,rc.top, pItem->m_Info.uDepth);
   //dump(m_pRoot);
